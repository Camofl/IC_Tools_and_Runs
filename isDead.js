        ...
        await page.evaluate(
            async (elements, revivedElements) => {
                ...
                async function isDead(element, checker = element, retries = 3) {
                    ...
                    const url = `/api/infinite-craft/pair?first=${encodeURIComponent(correctCasing)}&second=${encodeURIComponent(checker)}`;

                    for (let attempt = 0; attempt <= retries; attempt++) {
                        try {
                            const response = await fetchWithTimeout(url, 3000).then((x) => x.json());
                            let status = "alive";
                            if (response.result === "Nothing") status = "dead";
                            return status;
                        } catch (error) {
                            console.warn(`Attempt ${attempt + 1}: Failed to fetch for ${element} (${checker})`, error);
                            if (attempt === retries) {
